...
export const BranchSelector: React.FC<{
  ...
  const getBranches = useCallback(async () => {
    if (repositoryFullName && !isRequesting.current) {
      ...
      try {
        const branchesReq = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/users/github`, {
          body: JSON.stringify({
        ...
        if (branchesRes?.data?.length > 0) {
          ...
          if (!defaultBranch) {
            const fullRepo = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL}/api/users/github`, {
              body: JSON.stringify({
