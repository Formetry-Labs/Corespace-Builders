  ...
  const getDomainVerificationStatus = useCallback(
    async (domainId: string) => {
      ...
      const { status } = await fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL}/api/projects/${project?.id}/email-verification${
          query ? `?${query}` : ''
        }`,
        {
          credentials: 'include',
  ...
  const loadCustomDomainEmailAPIKey = useCallback(
    async (domainId: string) => {
      ...
      const { value } = await fetch(
        `${process.env.NEXT_PUBLIC_SITE_URL}/api/projects/${project?.id}/email-api-key${
          query ? `?${query}` : ''
        }`,
        {
          credentials: 'include',
    ...
    async (emailDomains: Props['emailDomain'][]) => {
      try {
        ...
        const req = await fetch(
          `${process.env.NEXT_PUBLIC_SITE_URL}/api/projects/${projectID}${
            query ? `?${query}` : ''
          }`,
          {
  ...
  const verifyEmailDomain = useCallback(
    async (domainId: string) => {
      try {
        ...
        const req = await fetch(
          `${process.env.NEXT_PUBLIC_SITE_URL}/api/projects/${projectID}/verify-email-domain${
            query ? `?${query}` : ''
          }`,
          {
